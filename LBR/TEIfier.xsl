...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//body/p">
        ...
        <xsl:variable name="deaccentifiedLemma">
            ...
            <xsl:value-of
                select="replace(replace(normalize-unicode($lemma, 'NFD'), '\p{IsCombining_Diacritical_Marks}', ''), '-', '')"
            />
        </xsl:variable>
    ...
    <xsl:template
        match="//text()[matches(normalize-space(.), '^\d')][preceding-sibling::*[1][self::hi[@rend = 'bold']]]">
